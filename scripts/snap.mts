...
const FIXTURE = {
  ...
  sessions: [
    { id: 'a1b2c3d4', title: 'Audit Layout Constants', repository: 'copilot-mission-control', branch: 'main', updated_at: new Date().toISOString(), is_active: true, status: 'working', event_count: 482, tool_count: 137, write_count: 24, read_count: 58, command_count: 22, web_count: 8, task_count: 11, error_count: 0, output_tokens: 64200, input_tokens: 184000, last_tool: 'apply_patch', last_event_kind: 'tool.execution_start', last_event_category: 'edits', stale_seconds: 4, git_root: '/Users/danwahlin/Desktop/projects/copilot-mission-control', recent_tool_calls: Array.from({ length: 14 }, (_, i) => ({ ts: new Date(Date.now() - i * 3000).toISOString(), tool: ['apply_patch', 'view', 'bash'][i % 3], category: ['edits', 'library', 'terminal'][i % 3], success: true })) },
  ...
  tools: [
    ...
    { name: 'apply_patch', category: 'edits', count: 38 },
  ...
  recent_events: Array.from({ length: 14 }, (_, i) => ({
    ...
    category: ['edits', 'library', 'terminal', 'library', 'signal', 'delegates'][i % 6],
    success: i !== 3,
  })),

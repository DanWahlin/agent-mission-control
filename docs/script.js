...
(function () {
    ...
    function guardLocalLiveServerReloads() {
        if (!['127.0.0.1', 'localhost'].includes(window.location.hostname) || !('WebSocket' in window))
            return;
        const NativeWebSocket = window.WebSocket;
        if (NativeWebSocket.__amcLiveServerGuarded)
            return;
  ...
  `;
    ...
    function maybeShootingStar(t) {
        if (t - lastShootingStar < 4000 || Math.random() > 0.002)
            return;
        lastShootingStar = t;
    ...
    function initCarousel() {
        ...
        if (!display || !label || !indicatorWrap)
            return;
        if (display.dataset.carouselInitialized === 'true')
            return;
